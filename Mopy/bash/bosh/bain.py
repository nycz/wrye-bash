...
class Installer(object):
    ...
    def _fixme_drop__for_loading_in_previous_versions(self):# FIXME: DROP backwards compat !
...
class InstallersData(DataStore):
        ...
        if last:
            ...
            for installer in self.itervalues():
                if installer.order > maxOrder and installer.isActive:
                    mask |= set(installer.data_sizeCrc) # lowercase !!!
    ...
    def _is_ini_tweak(ci_relPath):
        return ci_relPath[-4:].lower() == u'.ini' \
           and ci_relPath[:11].lower() == u'ini tweaks/' \
           and not os.path.sep in ci_relPath[11:]
